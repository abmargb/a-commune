...
public class XMPPProtocol extends Protocol implements PacketListener{
	...
	public void start() {
		...
		AndroidConnectionConfiguration cc = null;
		try {
			cc = new AndroidConnectionConfiguration(serverName, serverPort);
		} catch (XMPPException e1) {
			if(connectionListener != null){
				connectionListener.connectionFailed(
						new CommuneNetworkException("Could not create configuration", e1));
			}
			return;
		}
			...
			public void connectionCreated() {
				...
				if (checkResource) {
					try {
						...
						PingUtils.checkResource(pingCc, login, password, resource, getSleepTime());
